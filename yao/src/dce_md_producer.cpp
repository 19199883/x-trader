...
DceMDProducer::DceMDProducer(struct vrt_queue  *queue)
:module_name_("DceMDProducer")
{
    ...
    thread_rev_ = new std::thread(&DceMDProducer::RevData, this);
}

void DceMDProducer::ParseConfig()
{
...
int32_t DceMDProducer::Push(const MDBestAndDeep_MY& md){
...
MDBestAndDeep_MY* DceMDProducer::GetBestAnddeep(int32_t index)
{
	return &bestanddeep_buffer_[index];
}

int32_t DceMDProducer::Push(const MDOrderStatistic_MY& md){
...
MDOrderStatistic_MY* DceMDProducer::GetOrderStatistic(int32_t index)
{
	return &orderstatistic_buffer_[index];
}

bool DceMDProducer::IsDominant(const char *contract)
{
	return IsDominantImp(contract, dominant_contracts_, dominant_contract_count_);
}

// lic
MDBestAndDeep_MY* DceMDProducer::GetLastDataForIllegaluser(const char *contract)
{
